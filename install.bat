...
REM Already inside the app folder? then just install deps.
if exist "setup\install.ps1" ( echo Detected app folder, installing dependencies... & goto deps )

where git >nul 2>nul
if %errorlevel%==0 (
  echo [1/2] Cloning Meta Wingman from Gitee ...
...
:zip
echo [1/2] git not found, downloading zip from Gitee ...
...
if errorlevel 1 ( echo Download failed. Check network, or download manually from https://gitee.com/fsy2004/meta-wingman & pause & exit /b 1 )
...
:deps
echo.
echo [2/2] Installing R / Python dependencies (Tsinghua mirror) ...
