...
def _resolve_r_exe():
    """解析 Rscript 绝对路径:在 PATH 上则返 None(裸名即可);不在 PATH(R 默认安装不加 PATH)
    则用 env_check 的离线发现补出绝对路径,避免面板显示就绪但每次运行都 127。"""
    ...
    try:
        ...
        import env_check
        return env_check._find_r()
    except Exception:
        return None


_R_EXE = _resolve_r_exe()


def _resolve_interp(interp: str) -> str:
    """把裸 'Rscript' 换成绝对路径(当 R 不在 PATH 时);否则运行时预检会误判为缺失并报 127。"""
    if _R_EXE and interp.lower() in ("rscript", "rscript.exe") and not shutil.which(interp):
        return _R_EXE
    return interp
...
def api_run(req: RunReq):
    ...
    argv[0] = _resolve_interp(argv[0])          # R 不在 PATH 时换成绝对路径
...
def api_file(path: str):
    """安全地回传产物图/输入文件(仅限 runs 目录与示例数据目录)。
    ★校验一律在任何文件系统访问之前完成:
      - 拒绝 NUL 与 UNC 路径(\\host\\share 会让 Windows 发起 SMB 外连,泄露 NTLM 凭据);
      - 用纯字符串规整(os.path.abspath,不用 Path.resolve —— 后者在 Windows 上会 stat/触发外连);
      - 先过白名单,最后才 exists()/回传。"""
    if "\x00" in path:
        raise HTTPException(400, "非法路径")
    if path.startswith("\\\\") or path.startswith("//"):
        raise HTTPException(403, "不允许 UNC 路径")
    try:
        p = Path(os.path.abspath(path))          # 纯路径规整,不碰文件系统
    except Exception:
        raise HTTPException(400, "非法路径")
    if str(p).startswith("\\\\") or p.drive.startswith("\\\\"):
        raise HTTPException(403, "不允许 UNC 路径")
    # ★精确目录边界,且在 exists() 之前:未授权路径绝不触碰文件系统
    if not any(_within(p, a) for a in _FILE_ALLOWED):
...
def api_envinstall(req: EnvInstallReq):    # ★body 必填:强制 JSON→跨域 POST 触发预检被 CORS 拦,防 CSRF 触发装机脚本
